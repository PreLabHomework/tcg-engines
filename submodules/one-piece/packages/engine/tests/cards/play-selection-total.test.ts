...
/**
 * Soundness of PlayAction.totalConstraint for play selections.
 *
 * Printed text of the form "play up to N cards ... with a total cost of X or
 * less" constrains the chosen SET, not each card. Three constraints must hold
 * together: cardinality (<= 2), pairwise uniqueness (differentNames) and the
 * aggregate budget (totalConstraint, cost <= 9).
 *
 * Two properties are pinned beyond the budget itself:
 *  - Refusal at submission. An over-budget combination must be rejected when
 *    the selection is submitted, so the player can choose again. Rejecting it
 *    later, during execution, closes the prompt and silently plays nothing.
 *  - Prompt legality. Every offered option must belong to at least one legal
 *    selection. With an upper-bound total, a card whose own cost already
 *    exceeds the budget can never be part of a legal set and is not offered.
 */
        ...
        actions: [
          {
            ...
            differentNames: true,
            totalConstraint: { property: "cost", comparison: "lte", value: 9 },
...
describe("PlayAction.totalConstraint", () => {
  const setup = () => {
