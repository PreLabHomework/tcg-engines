...
import { getCard, op13Higuma013, op13MonkeyDLuffy001, op13Otama043 } from "@tcg/op-cards";
...
const DEFAULT_LEADER = op13MonkeyDLuffy001;
const FILLER_CARD = op13Otama043;
const ATTACKER_CARD = op13Higuma013;
...
function printedTextMentionsTrigger(card: OPCard, trigger: EffectTrigger): boolean {
  ...
  const triggerPatterns: Record<EffectTrigger, RegExp> = {
    ...
    startOfYourTurn: /at the start of your turn/i,
    ...
    whenYouDealDamage: /when you deal damage/i,
    whenCharacterKod: /Character .*K\.O\.'d|K\.O\.s? .*Character|when.*K\.O\./i,
    whenCharacterRemoved:
      /Character .*removed from the field|Character .*K\.O\.'d|Character .*leaves the field/i,
    whenLeaving:
    ...
    whenCardTrashedFromHandByEffect: /card is trashed from your hand by .*effect/i,
...
function printedTextMentionsAction(card: OPCard, action: string): boolean {
  ...
  const actionPatterns: Record<string, RegExp> = {
    ...
    giveDonFromDonPhase: /DON!! card placed during your DON!! Phase is given/i,
    grantKeyword: /gains? \[|gain .*during/i,
    guessTopDeckCost: /choose a cost.*reveal/i,
    ko: /K\.?O\.?/i,
    lookAtLife: /look at .*Life/i,
    modifyCost: /cost/i,
    modifyPower: /power/i,
    negateEffects: /negate .*effect|effects? .*negated/i,
    negatePlayerEffects: /negate .*effect|effects? .*negated/i,
    opponentReturnDon: /return .*DON!!|DON!! cards? .*DON!! deck/i,
    play: /play/i,
    playRested: /played rested|play .*rested/i,
    playRestriction: /cannot play|can't play|play/i,
    rearrangeDeck: /look at|reveal|top|bottom|order|deck/i,
    rearrangeLife: /look at all .*Life|Life area in any order/i,
    ...
    swapBasePower: /swap .*power|base power/i,
    copyPower: /base power becomes the same|copy .*power/i,
...
function playFromHand(card: OPCard): MatchState {
  ...
  engine.playCard(card, "south", card.cardType === "character" ? 0 : undefined);
...
function useCounterEvent(card: OPCard): MatchState {
  const engine = OnePieceTestEngine.create(
    ...
    { leaderCardId: DEFAULT_LEADER, hand: [card], deck: [FILLER_CARD], life: 4 },
  );
  drainPrompts(engine);
  engine.endTurn("south");
  drainPrompts(engine);
...
function useLifeTrigger(card: OPCard): MatchState {
  const engine = OnePieceTestEngine.create(
    { leaderCardId: DEFAULT_LEADER, deck: [FILLER_CARD], life: 4, activeDon: 1 },
    {
      ...
      life: [card, FILLER_CARD, FILLER_CARD, FILLER_CARD, FILLER_CARD],
      deck: [FILLER_CARD],
    },
  );
  drainPrompts(engine);
